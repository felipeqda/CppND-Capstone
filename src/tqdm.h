...
// credit: https://github.com/aminnj/cpptqdm
// modifications to customize bar display on init
...
class tqdm {
    private:
        ...
        std::string label = "";
        std::string title = "";
        std::string f_unit = "Hz";
        std::string ETA_format = "%.0fs<%.0fs";  // printf-compatible format as std::string
    ...
    public:
        ...
        void set_funit(std::string f_unit_)        { f_unit = f_unit_; }
        void set_ETAformat(std::string eta_format) { ETA_format = eta_format; }
        ...
        void progress(int curr, int tot) {
            if(is_tty && (curr%period == 0)) {
                ...
                double div = 1.;
                if (avgrate > 1e6) {
                    f_unit = "M"+f_unit; div = 1.0e6;
                } else if (avgrate > 1e3) {
                    f_unit = "k"+f_unit; div = 1.0e3;
                }
                printf("[%4d/%4d ", curr, tot);
                if (!f_unit.empty()) printf("| %3.1f %s ", avgrate/div, f_unit.c_str());
                if (!ETA_format.empty()) {
                    printf("| ");
                    printf(ETA_format.c_str(), dt_tot, peta); 
                }
